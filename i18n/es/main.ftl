...
book-email-invalid = Introduce una dirección de correo completa, incluido el dominio (p. ej. jane@example.com).
...
book-phone-invalid = Introduce un número al que podamos enviar SMS, o deja el campo vacío.
...
register-submit = Crear la cuenta
register-have-account = ¿Ya tienes una cuenta? { $link }
register-signin-link = Inicia sesión

# Authentication errors (src/auth.rs)

auth-error-rate-limited = Demasiados intentos de inicio de sesión. Inténtalo de nuevo más tarde.
...
auth-error-email-invalid = Introduce una dirección de correo válida
...
form-error-slug-reserved = Este identificador está reservado. Elige otro.
...
form-error-no-account = No se ha encontrado ningún perfil de agenda. Habla con administración.
...
settings-error-booking-email = Introduce una dirección de correo de reservas válida.
...
error-oauth-invalid-state = Parámetro de estado no válido. Inténtalo de nuevo.
